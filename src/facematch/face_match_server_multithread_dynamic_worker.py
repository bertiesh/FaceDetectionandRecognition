    ...
    def get_worker_count(self):
        ...
        with self.lock:
            ...
            if memory_percent > 85 and new_worker_count > self.min_workers:
                new_worker_count -= 1
                log_info(f"Reducing workers due to high memory usage ({memory_percent}%)")
            elif available_memory_gb < 1.0 and new_worker_count > self.min_workers:
                new_worker_count -= 1
                log_info(f"Reducing workers due to low available memory ({available_memory_gb:.2f} GB)")
                
            # Adjust based on GPU if available
            if gpu_utilization > 90 and new_worker_count > self.min_workers:
                new_worker_count -= 1
                log_info(f"Reducing workers due to high GPU utilization ({gpu_utilization}%)")
                
            # Update current workers
            if new_worker_count != self.current_workers:
                log_info(f"Adjusting worker count from {self.current_workers} to {new_worker_count}")
...
def find_face_endpoint(
    ...
    if not health_monitor.is_healthy():
        return ResponseBody(root=TextResponse(
    ...
    input_files_with_complexity = [
        (path, estimate_image_complexity(path)) 
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=worker_count) as executor:
        ...
        future_to_image = {
            executor.submit(
                ...
                database_path
            ): image_path for image_path in sorted_input_files
        }
            ...
            try:
                ...
                if status:
                    if isinstance(results, list):
                        ...
                        log_info(f"Unexpected result type for {image_path}: {type(results)}")
                else:
