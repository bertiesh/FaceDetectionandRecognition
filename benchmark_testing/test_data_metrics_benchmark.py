...
parser.add_argument(
    "--results_path", required=False, type=str, help="Parent Results directory", default="./benchmark-results"
)
...
def calc_true_label():
    ...
    queries, db_images = (list(map(lambda x: '_'.join(os.path.basename(x).split('_')[:-1]),path_list)) for path_list in (os.listdir(queries_path), os.listdir(db_images_path)))
...
benchmark_results_dir = os.path.join(abs_results_path, f"{detector_backend}-{model_name}-{args.results_name}",'results-csv')
output_directory = os.path.join(abs_results_path, f"{detector_backend}-{model_name}-{args.results_name}", "output-csv-dump")

for top_n, n in zip(top_n, N):
    # benchmark results file path
    benchmark_results_path = os.path.join(benchmark_results_dir, top_n, f'aggregate_results_{top_n}.csv')
    ...
    for filename in output_files:
        ...
        results = [detector_backend, model_name, similarity_threshold, accuracy, precision, recall, f1, tp, fp, tn, fn]
        ...
        detailed_results_path = os.path.join(benchmark_results_dir, top_n, "detailed_results",f"{detector_backend}_{model_name}_{similarity_threshold}_results.txt")
        ...
        detailed_metrics_path = os.path.join(benchmark_results_dir, top_n, "detailed_metrics",f"{detector_backend}_{model_name}_{similarity_threshold}_metrics.txt")
...
generic_detailed_metrics_path = os.path.join(benchmark_results_dir,"top_n","detailed_metrics")
generic_detailed_results_path = os.path.join(benchmark_results_dir,"top-n","detailed_results")
