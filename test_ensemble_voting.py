...
def weighted_majority_vote(all_results, weights):
    ...
    for image_name, vote_sum in weighted_votes.items():
        ...
        total_possible_weight = sum(weights.get(file, 1.0) for file in all_results.keys() 
...
def main():
    ...
    weights = {
        ...
        'yolov8_ArcFace_0.48_results.txt': .9,
    }
    ...
    print(f"Correctly classified: {correctly_classified} ({correctly_classified/len(final_results)*100:.2f}%)")
    incorrectly_classified = sum(1 for result in final_results.values() if result == 0)
    print(f"Incorrectly classified: {incorrectly_classified} ({incorrectly_classified/len(final_results)*100:.2f}%)")
    ...
    return {
        ...
        "summary": {
            ...
            "accuracy": correctly_classified/len(final_results) if final_results else 0
        }
    }
